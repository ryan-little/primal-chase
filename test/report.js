...
const STRATEGY_LABELS = {
  'random': 'Random', 'newbie': 'Newbie', 'intermediate': 'Intermediate',
  'smart': 'Smart', 'gto': 'GTO'
};
...
function run() {
  ...
  // Handle difficulty-nested structure (--difficulty=all produces { easy: {...}, normal: {...}, hard: {...} })
  const isDifficultyNested = data.easy || data.normal || data.hard;
  let report;
  if (isDifficultyNested) {
    const parts = [];
    for (const [diff, stratData] of Object.entries(data)) {
      parts.push(`\n${'#'.repeat(60)}\n  DIFFICULTY: ${diff.toUpperCase()}\n${'#'.repeat(60)}`);
      parts.push(generateReport(stratData));
    }
    report = parts.join('\n');
  } else {
