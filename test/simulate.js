...
// Usage: node test/simulate.js [--games=500] [--strategy=all|random|newbie|intermediate|smart|gto] [--difficulty=all|easy|normal|hard]
// ============================================================
...
const STRATEGIES = {
  'random':       { label: 'Random',       picker: 'random' },
  'newbie':       { label: 'Newbie',        picker: 'newbie' },
  'intermediate': { label: 'Intermediate',  picker: 'intermediate' },
  'smart':        { label: 'Smart',         picker: 'smart' },
  'gto':          { label: 'GTO',           picker: 'gto' }
};
...
function pickAction(actions, strategy, gameState, sandbox) {
  ...
  const picker = STRATEGIES[strategy]?.picker || strategy;
  switch (picker) {
    case 'random':       return randomPick(actions);
    case 'newbie':       return newbiePick(actions, gameState);
    case 'intermediate': return intermediatePick(actions, gameState);
    case 'smart':        return smartPick(actions, gameState);
    case 'gto':          return gtoPick(actions, gameState, sandbox);
    default:             return actions[0];
  }
}

// ---- RANDOM: pure uniform random choice ----
function randomPick(actions) {
  return actions[Math.floor(Math.random() * actions.length)];
}

// ---- NEWBIE: panic instinct, always runs, grabs food/water eagerly ----
function newbiePick(actions, state) {
  const standard = {};
  const situational = [];
  ...
  // Signature encounters: newbie picks first option (doesn't analyze)
  if (!standard.push && !standard.trot && !standard.rest) {
    return actions[0];
  }

  // Always grab drink/eat if available (even if not needed — greedy)
  ...
  // Panic: always push (run from hunters!)
  if (standard.push) return standard.push;

  // Fallback to trot
  return standard.trot || actions[0];
}

// ---- INTERMEDIATE: knows basics, trots by default, basic day/night sense ----
function intermediatePick(actions, state) {
  const standard = {};
  ...
  // Signature encounters: basic analysis
  if (!standard.push && !standard.trot && !standard.rest) {
    return pickBestSignatureAction(actions, state);
  }

  // Drink when thirsty
  if (state.thirst >= 50 && situational.drink) return situational.drink;
  // Eat when hungry
  if (state.hunger >= 60 && situational.eat) return situational.eat;
  // Rest when tired or overheating
  if (state.stamina <= 35 && standard.rest) return standard.rest;
  if (state.heat >= 70 && standard.rest) return standard.rest;
  // Push during day when healthy
  if (state.phase === 'day' && state.stamina >= 60 && state.heat <= 40 && standard.push) return standard.push;
  // Rest at night when not fresh (basic day/night awareness)
  if (state.phase === 'night' && state.stamina <= 60 && standard.rest) return standard.rest;
  // Grab food/water if moderately needed
  if (state.thirst >= 35 && situational.drink) return situational.drink;
  if (state.hunger >= 45 && situational.eat) return situational.eat;
  // Default to trot
  return standard.trot || actions[0];
}
